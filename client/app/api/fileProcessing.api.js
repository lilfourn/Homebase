...
export const processGoogleDriveFile = async (fileId, fileName, token) => {
  try {
    ...
    if (error.response?.status === 401 && errorData?.errorCode === "GOOGLE_INVALID_GRANT") {
      // Create a specific error object for this case
      const authError = new Error(errorData.error || "Google Drive authentication is invalid");
 ...
 */
export const uploadTerminalFile = async (file, token) => {
  try {
    const formData = new FormData();
    formData.append('file', file);
