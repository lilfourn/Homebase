...
import { FileText, File, Image, Video, Upload, Check, Loader2, AlertCircle } from 'lucide-react';
...
export default function FileLibrary({ 
  ...
  const { openPicker, isLoading: isUploading } = useGoogleDrivePicker({
    ...
    onFilesSelected: (result) => {
      setSuccess(result.message || `${result.files?.length || 0} file(s) imported successfully.`);
      setError('');
  ...
  const files: FileItem[] = courseFiles ? courseFiles.map(f => ({
    id: f.id,
    name: f.name,
    type: f.type,
    size: f.size,
    ...
    <div className="space-y-4">
      ...
      {filesLoading ? (
        ...
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-xs mt-1">Upload files using the button below to get started.</p>
        </div>
      ) : (
          ...
          return (
            <div
              ...
              style={{
                ...
                backgroundColor: isSelected ? courseColors.primary + '10' : undefined
              }}
            >
              <div className="flex items-center gap-3">
                <div 
                  ...
                  style={{ 
                    backgroundColor: isSelected ? courseColors.primary + '20' : '#f3f4f6',
              ...
              {isSelected && (
                <Check className="w-5 h-5" style={{ color: courseColors.primary }} />
              )}
            </div>
          );
        })}
      ...
      <div
        className={`
          ...
          ${dragActive ? 'border-opacity-100 bg-opacity-10' : 'border-opacity-50'}
          ${isDisabled || isUploading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:border-opacity-75'}
        `}
        style={{
      ...
      >
        {isUploading ? (
          <Loader2 className="w-8 h-8 mx-auto mb-2 animate-spin" style={{ color: courseColors.primary }} />
        ) : (
          <Upload className="w-8 h-8 mx-auto mb-2" style={{ color: courseColors.primary }} />
        )}
        <p className="text-sm font-medium text-gray-700">
          {isUploading ? 'Opening Google Drive...' : 'Click to upload from Google Drive'}
        </p>
        <p className="text-xs text-gray-500 mt-1">
          {isUploading ? 'Please select files from the picker' : 'Add files to your course library'}
        </p>
        <button
