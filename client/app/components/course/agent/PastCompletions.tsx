...
import { ChevronDown, ChevronUp, CheckCircle, XCircle, Clock, FileText, Download, Eye } from 'lucide-react';
...
export default function PastCompletions({ completions, courseColors }: PastCompletionsProps) {
  ...
  React.useEffect(() => {
    console.log('[PastCompletions] Completions updated', { 
      count: completions.length,
      latestCompletion: completions[0] 
    });
  }, [completions]);
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'processing':
        return <Clock className="w-5 h-5 animate-spin" style={{ color: courseColors.primary }} />;
      default:
        return null;
    }
  };
      ...
      {completions.map((completion) => {
        ...
        return (
          <div
            ...
            style={{
              borderColor: completion.status === 'processing' ? courseColors.primary + '40' : '#e5e7eb'
            }}
          >
            {/* Header */}
            <div
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500 mt-0.5">
                    <span className="px-2 py-0.5 rounded-full" style={{ backgroundColor: courseColors.primary + '20', color: courseColors.primary }}>
                      {getAgentTypeLabel(completion.agentType)}
                    </span>
                    <span>•</span>
                    <span>{formatDate(completion.completedAt)}</span>
                    <span>•</span>
                    <span>{completion.files.length} file{completion.files.length !== 1 ? 's' : ''}</span>
                  </div>
                </div>
              </div>
            ...
            {isExpanded && (
              <div className="px-4 py-3 border-t bg-gray-50">
                <div className="space-y-3">
                  {/* Files Used */}
                  <div>
                    <h5 className="text-xs font-medium text-gray-700 mb-2">Files Used:</h5>
                  ...
                  {completion.status === 'processing' && (
                    <div className="flex items-center gap-2 text-sm" style={{ color: courseColors.primary }}>
