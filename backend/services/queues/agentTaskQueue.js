...
class AgentTaskQueue {
  ...
  async handleTimeout(job) {
    ...
    try {
      await convexService.updateTaskStatus({
        ...
        status: 'failed',
        error: {
          message: 'Task processing timed out. Please try with smaller files or simpler requests.',
          code: 'TIMEOUT_ERROR'
        },
        failedAt: new Date().toISOString()
      });
    } catch (error) {
      console.error(`[Queue] Failed to update timeout status for task ${taskId}:`, error);
    }
  }
  ...
  async addTask(taskData, options = {}) {
    ...
    const job = await this.queue.add('process-agent-task', taskData, jobOptions);
    return job.id;
  }
  ...
  calculateTimeout(taskData) {
    ...
    console.log(`[Queue] Calculated timeout for ${agentType}: ${totalTimeout}ms (${totalTimeout/1000}s)`);
